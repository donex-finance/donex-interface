...
export function getSqrtRatioAtTick(tick: number) {
  ...
  let ratio =
    (absTick & 0x1) != 0 ? BigInt('0xfffcb933bd6fad37aa2d162d1a594001') : BigInt('0x100000000000000000000000000000000') // 1 / sqrt(1.0001)
  if ((absTick & 0x2) != 0) ratio = (ratio * BigInt('0xfff97272373d413259a46990580e213a')) >> BigInt(128) // 1 / 1.0001
  if ((absTick & 0x4) != 0) ratio = (ratio * BigInt('0xfff2e50f5f656932ef12357cf3c7fdcc')) >> BigInt(128) // 1/1.0001^2
  ...
  if (tick > 0) {
    ...
    ratio = (BigInt(2 ** 256) - BigInt(1)) / ratio
  }
...
export function log_2(x: bigint) {
  ...
  if (msb > 128) x >>= BigInt(msb - 128)
  else if (msb < 128) x <<= BigInt(128 - msb)
...
/*export function getTickAtSqrtRatio(sqrtPriceX96: bigint) {
    ...
    const log2Ratio = log_2(ratio)
    // TODO: why?
    const log_sqrt10001 = log2Ratio * BigInt('255738958999603826347141') >> BigInt(64) // log_(sqrt(1.0001))(2) * 2 ** 64 = 255738958999603826347141

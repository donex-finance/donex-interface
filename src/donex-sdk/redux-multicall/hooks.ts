...
import { Contract, Contract as Interface } from 'starknet'
...
import { fromStarknetCall, toStarknetCall } from './utils/callUtils'
...
export function useCallsDataSubscription(
  ...
  useEffect(() => {
    if (listenerOptions?.disabled) return
...
export function useSingleContractMultipleData(
  ...
  const { fragment, callDatas } = useCallDatas(methodName, contract, callInputs)
  ...
  return useMemo(() => {
    if (!contract) return []
    return results.map((result) => toCallState(result, contract, fragment, latestBlockNumber))
...
export function useSingleContractWithCallData(
  ...
  return useMemo(() => {
    ...
    return results.map((result, i) => {
      const call = toStarknetCall({ address: '', callData: callDatas[i] })
      return toCallState(result, contract, call.entrypoint, latestBlockNumber)
    })
...
function useCallData(
  ...
  const fragment = methodName
  // Get encoded call data
  const callData: string | undefined = useMemo(() => {
    if (!fragment || !isValidMethodArgs(callInputs) || !contractInterface) return undefined
    const call = contractInterface.populate(fragment, callInputs)
    return fromStarknetCall(call).callData
  }, [callInputs, contractInterface, fragment])
  return { fragment, callData }
}

function useCallDatas(
  methodName: string,
  contractInterface: Interface | null | undefined,
  callInputsArray: OptionalMethodInputs[] | undefined
) {
  // Create ethers function fragment
  const fragment = methodName
  // Get encoded call data
  const callDatas: string[] | undefined = useMemo(() => {
    if (!fragment || !callInputsArray || !isValidMethodArgs(callInputsArray) || !contractInterface) return []
    return callInputsArray.map((v) => {
      const starknetCall = contractInterface.populate(fragment, v)
      const call = fromStarknetCall(starknetCall)
      return call.callData
    })
  }, [callInputsArray, contractInterface, fragment])
  return { fragment, callDatas }
}

...
import { CHUNK_GAS_LIMIT } from './constants'
...
import { toStarknetCall } from './utils/callUtils'
import chunkCalls from './utils/chunkCalls'
import { retry, RetryableError } from './utils/retry'
import useDebounce from './utils/useDebounce'

type UniswapInterfaceMulticall = Contract
  ...
  try {
    if (chunk.length > 0) {
      const subFetch = async (params: any[]) => {
        ...
        try {
          const { result } = await multicall.callStatic.aggregate(params)
          for (let j = 1; j < result.length; j++) {
            subRet.returnTmps.push(result[j].toString())
          }
        } catch (e) {
          subRet.success = false
        }
        return subRet
      }
      ...
      for (let i = 0; i < chunk.length; i++) {
        const obj = chunk[i]
        const call = toStarknetCall(obj)
        const req = subFetch(
          [obj.address as any, getSelectorFromName(call.entrypoint), call.calldata?.length].concat(call.calldata)
        )
        arr.push(req)
      }
      ...
      for (let i = 0; i < arr.length; i++) {
        const { success, returnTmps } = await arr[i]
        returnDatas.push({
          success,
          returnData: returnTmps.join(','),
        })
      }
      return returnDatas
    } else {
      const ret: { success: boolean; returnTmps: string[] }[] = chunk.map((v) => ({ success: true, returnTmps: [] }))
      const { block_number, result } = await multicall.callStatic.aggregate(
        chunk
          .map((obj) => {
            const call = toStarknetCall(obj)
            return [obj.address as any, getSelectorFromName(call.entrypoint), call.calldata?.length].concat(
              call.calldata
            )
          })
      ...
      let index = 0
      let len = result[0].toNumber()
      for (let i = 1; i < result.length; i++) {
        if (len === 0) {
          len = result[i].toNumber()
          index++
          continue
        }
        if (len > 0) {
          ret[index].returnTmps.push(result[i].toString())
          len--
        }
      }
      const returnDatas = []
      for (let i = 0; i < ret.length; i++) {
        const { success, returnTmps } = ret[i]
        returnDatas.push({
          success,
          returnData: returnTmps.join(','),
        })

...
import { ETH_MAINNET, NATIVE_CHAIN_ID, WRAPPED_NATIVE_CURRENCY } from 'constants/tokens'
import { Currency, CurrencyAmount, NativeCurrency, Token } from 'donex-sdk/sdk-core'
          ...
          if (wrappedNative) {
            promises.push(
              new Promise(async (resolve) => {
                try {
                  const wrappedNativeContract = getContract(ETH_MAINNET.address, ERC20_ABI, provider)
                  const balance = await wrappedNativeContract.balanceOf(ownerAddress)
